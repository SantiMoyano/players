...
userCtrl.createUser = async (req, res) => {
  try {
    ...
    const profileBio = `Welcome to ${username}'s profile!`;
...
userCtrl.addFavouritePlayer = async (req, res) => {
  try {
    ...
    res.json({ message: "Favorite player added to the user" });
  } catch (error) {
    res.status(500).json({ message: "Error adding favorite player" });
  }
};
...
userCtrl.removeFavouritePlayer = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: "Error removing favorite player" });
  }
};
